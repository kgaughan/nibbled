...
# build the binary
build:
	go build -v -tags netgo -trimpath -ldflags '-s -w' -o nibbled .
...
clean:
	...
	rm -rf nibbled dist site coverage.out coverage.html
...
[group('testing')]
test-release:
	goreleaser release --auto-snapshot --clean --skip ko --skip homebrew --skip publish
